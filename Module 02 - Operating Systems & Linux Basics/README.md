...
- `pwd` = Show current directory. Example Output: `/home/devops`
...
- `cd /home/devops` = Go to my home directory (absolute path)
...
- `cat /etc/*release` =  Show OS-related information
...
- `su - admin` = Switch from devops user to admin
</details>
...
echo "you have entered $folder_name"
```
...
`./example.sh name lastname # 2 params here`

`./example.sh "name lastname" # just 1 param`
...
- `HOME=/home/devops`= current user's home directory
- `USER=devops` = currently logged in user
<!-- -->
...
- `PATH=$PATH:/home/devops` = Appending /home/devops folder to the existing $PATH value
...
- `ssh root@1.1.1.1`= Connect with root user to 1.1.1.1 server address
...
- `ssh -i .ssh/id_rsa root@1.1.1.1` = Connect with root user to 1.1.1.1 server address with specified private key file location (.ssh/id_rsa = default, but you can specify a different one like this)
